...
export const Skeleton = ({ icons }: SkeletonProps) => {
  const scale = [1, 1.1, 1];
  ...
  console.log("Icons-----");
  console.log(icons[3]?.icon);
  return (
    <div className="relative flex h-full items-center justify-center overflow-hidden p-8">
      <div className="flex flex-shrink-0 flex-row items-center justify-center gap-2">
        <Container className="circle-1 h-8 w-8">{icons[0]?.icon}</Container>
        <Container className="circle-2 h-12 w-12">{icons[1]?.icon}</Container>
        <Container className="circle-3">{icons[2]?.icon}</Container>
        <Container className="circle-4 h-12 w-12">{icons[3]?.icon}</Container>
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "group mx-auto w-full max-w-sm rounded-xl border border-[rgba(255,255,255,0.10)] bg-gray-100 p-8 shadow-[2px_4px_16px_0px_rgba(248,248,248,0.06)_inset] dark:bg-[rgba(25,25,25,0.7)]",
        className,
      )}
    >
      {children}
